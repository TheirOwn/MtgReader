...
def printExcel2(cards,traits, wbname, mode="commander"):
...
def main():
    ...
    while user.lower() != "start search":
        user = input("Input a word or exact phrase to search for, or enter " +\
                     "\"start search\" to begin search")
        searchtags.append(user.lower())
    file = r"C:\Users\Nicholas\Documents\Mtg\AllCards.json"
    level = 0
    tag = ""
    body = ""
    cards = []
        ...
        for line in f:
            ...
            if len(line) == 0:
                ...
                if level == 0 and line[0] == '"': #name of the card
                    ...
                    level -= 1
    f.close()
    print("Finished loading. Now searching through cards")
    ...
    for x in cards:
        if x.search("Merfolk"):
            save.append(x)
    mode = "commander"
    printExcel2(save,["name","manaCost", "alltypes","text","power","toughness","colors","legalities"],"ReaderTest.xlsx",mode)
    for x in save:
        print(x)
